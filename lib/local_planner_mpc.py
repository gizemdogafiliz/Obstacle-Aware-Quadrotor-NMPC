...
================================================================
Quaternion NMPC with input box constraints, per-rotor feasibility, landing
cone and soft obstacle keep-out constraints. Solved online with acados
SQP-RTI + HPIPM at N=20, Ts=50 ms.
...
try:
    ...
    from lib.quadrotor_model import export_quadrotor_model
...
_CODEGEN_DIR = os.path.join(_PROJECT_ROOT, 'acados_generated')
...
def _clean_generated():
    ...
    for d in [_CODEGEN_DIR,
              os.path.join(_PROJECT_ROOT, 'c_generated_code')]:
        if os.path.isdir(d):
...
class LocalPlannerMPC:
    ...
    def __init__(self, N=10, Ts=0.05, mass=1.28,
             ...
             motor_limits=True, kf=6.5e-4, km=1e-5, arm=0.23, W_motor=100000.0,
        ...
        ocp = self._build_ocp(N, Ts, mass, I_diag, n_obs_max, R_drone,
                               ...
                               motor_limits, kf, km, arm, W_motor)
    ...
    def _build_ocp(N, Ts, mass, I_diag, n_obs_max, R_drone,
                   ...
                   alpha_land, W_land, rk4_steps,
                   motor_limits=False, kf=6.5e-4, km=1e-5, arm=0.23,
                   W_motor=100000.0):
        ocp   = AcadosOcp()
        ...
        h_list     = []
        soft_h_idx = []   # indices into h_list that are softened
        soft_pen   = []   # their slack penalties

        # Obstacle avoidance constraints (soft, W_obs)
        for i in range(n_obs_max):
            ...
            soft_h_idx.append(len(h_list) - 1);  soft_pen.append(W_obs)

        # Motor feasibility: each rotor's squared speed omega_i^2 >= 0  (HARD).
        # omega^2 = B^-1 @ u is LINEAR in u = [f, tau_x, tau_y, tau_z], so HPIPM
        # enforces it exactly. The NMPC can then never command a wrench the four
        # motors cannot realise -> the after-the-fact mixer clip is unnecessary.
        n_motor = 0
        if motor_limits:
            B = np.array([
                [  kf,       kf,       kf,       kf    ],
            ...
            omega_sq = cs.mtimes(cs.DM(B_inv), u)   # 4x1, omega_i^2
            for j in range(4):
                h_list.append(omega_sq[j])          # hard: not added to soft set
            n_motor = 4

        # Landing cone constraint: vz + alpha*z >= 0  (soft, W_land)
        has_land = alpha_land > 0
        if has_land:
            h_list.append(x[5] + alpha_land * x[2])
            soft_h_idx.append(len(h_list) - 1);  soft_pen.append(W_land)
        ...
        # Soften ONLY obstacle + landing; motor limits stay HARD (no slack column)
        ns  = len(soft_h_idx)
        Jsh = np.zeros((nsh, ns))
        for k, hidx in enumerate(soft_h_idx):
            Jsh[hidx, k] = 1.0
        ocp.constraints.Jsh = Jsh
        ocp.cost.zl = np.zeros(ns)
        ocp.cost.zu = np.zeros(ns)
        ocp.cost.Zl = np.array(soft_pen, dtype=float)
        ocp.cost.Zu = np.zeros(ns)
