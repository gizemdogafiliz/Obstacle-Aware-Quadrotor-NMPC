...
quad_nmpc_client.py
===================
Public API for the obstacle-aware NMPC quadrotor. Wraps the acados NMPC
(LocalPlannerMPC), the APF/homotopy/goal-only reference generators, and
the lidar+DBSCAN perception pipeline into interactive commands
(hover, set_position, landing, waypoints, slalom variants).
"""
...
from lib.local_planner_mpc import LocalPlannerMPC
from lib.global_planner import WaypointTrajectory, BackflipTrajectory, APFTrajectory
from lib.perception import PerceptionManager
...
_mixer_warn_t = 0.0
_MIXER_VERBOSE = os.environ.get('NMPC_MIXER_VERBOSE', '0') == '1'
...
def wrench_to_rotorcraft(f, tau_x, tau_y, tau_z):
    ...
    if _MIXER_VERBOSE and np.any(omega_sq < 0):
        now = time.time()
...
# INTERACTIVE MENU
# ═══════════════════════════════════════════════════════════════════════════
# Numbered scenarios that match the README "Testing and Usage" section.
# Each entry carries the Terminal 1 launch command it expects and the level
# passed to setup(); the callable runs the scenario itself.
# A blank input or Ctrl+C drops straight to the >>> prompt without doing
# anything, so power users can bypass the menu and drive the API by hand.

def _ensure_setup(perception_level: int):
    """Call setup(perception_level) only if the stack has not been armed yet."""
    global _mpc
    if _mpc is None:
        setup(perception_level=perception_level)
...
_SCENARIOS = [
    ("HOVER",
     "simulation.sh",
     3,
     lambda: hover(0, 0, 4, T_hover=10, log_tag='hover')),
    ("WAYPOINTS",
     "simulation.sh",
     3,
     lambda: follow_waypoints(
         [[0, 0, 3], [4, 0.5, 3], [8, -0.5, 3], [0, 0, 3]],
         max_vel=0.8, log_tag='waypoints')),
    ("BACKFLIP",
     "simulation.sh",
     3,
     lambda: backflip_mpc_recovery()),
    ("APF-OFFLINE",
     "simulation_obstacles.sh",
     3,
     lambda: slalom(max_vel=0.5)),
    ("APF-ONLINE (dense)",
     "simulation_obstacles.sh worlds/quad_obstacles_dense.world",
     2,
     lambda: slalom_reactive(use_perception=True, max_vel=2.0)),
    ("PURE-NMPC (dense)",
     "simulation_obstacles.sh worlds/quad_obstacles_dense.world",
     2,
     lambda: slalom_mpc_avoid(max_vel=1.0)),
    ("COMPARISON A-D (line)",
     "simulation_obstacles.sh worlds/quad_obstacles_line.world",
     2,
     None),   # sub-menu
]

_COMPARISON = [
    ("A - pure-MPC (in-MPC keep-out)",
     lambda: slalom_mpc_avoid(max_vel=1.3)),
    ("B - APF tracker tight",
     lambda: slalom_reactive(use_perception=True, max_vel=2.2,
                              apf_d0=0.3, apf_R_drone=0.20)),
    ("C - APF wide",
     lambda: slalom_reactive(use_perception=True, max_vel=2.81)),
    ("D - pure-MPC wide",
     lambda: slalom_mpc_avoid(max_vel=0.5, safety_margin=1.10, T_timeout=100.0)),
]
...
def menu():
    """Interactive scenario picker; also runs on `python3 -i` startup."""
    print("\n" + "=" * 72)
    print("  NMPC Quadrotor scenarios")
    print("=" * 72)
    print("  Terminal 1 must be running the matching simulation stack.\n")
    for i, (name, term1, _lvl, _fn) in enumerate(_SCENARIOS, 1):
        print(f"  {i}) {name:<22s}  [Term1: {term1}]")
    print(f"  0) skip menu (drop to interactive >>> prompt)\n")

    raw = _prompt(f">> Choose scenario [0-{len(_SCENARIOS)}]: ")
    ...
    try:
        ...
        print(f"[menu] Invalid input: '{raw}'")
        return
    if not (0 <= idx < len(_SCENARIOS)):
        print(f"[menu] Out of range: {raw}")
        return

    name, term1, lvl, fn = _SCENARIOS[idx]
    print(f"\n[menu] {name}   (Terminal 1: {term1})")

    if name.startswith('COMPARISON'):
        print("  Sub-configurations (share one setup):")
        for j, (sub_name, _sfn) in enumerate(_COMPARISON, 1):
            print(f"  {j}) {sub_name}")
        sub = _prompt(f">> Choose configuration [1-{len(_COMPARISON)}]: ")
        ...
        try:
            ...
            print(f"[menu] Invalid input: '{sub}'")
            return
        if not (0 <= sidx < len(_COMPARISON)):
            print(f"[menu] Out of range: {sub}")
            return
        _ensure_setup(lvl)
        _COMPARISON[sidx][1]()
        return
